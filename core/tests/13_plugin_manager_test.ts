...
import { IPlugin, EventPayload, IAttribution, IEventDoc, IMetricUpdate, Granularity, IQuery, IReportDataPoint, AggregationType } from "../types.ts";
...
class TestTrackerPlugin implements IPlugin {
  ...
  beforeEventRecord = async (context: { payload: EventPayload; eventType: string; attributions?: IAttribution[] }) => {
    this.callTracker.beforeEventRecord++;
    return context;
  };

  afterEventRecord = async (_context: { eventDoc: IEventDoc<EventPayload> }) => {
    this.callTracker.afterEventRecord++;
  };

  onGetMetrics = async (_context: { eventDoc: IEventDoc<EventPayload>; storageGranularity: Granularity }) => {
    this.callTracker.onGetMetrics++;
    return [] as IMetricUpdate[];
  };

  beforeMetricsWritten = async (context: { metrics: IMetricUpdate[]; targetCollection: string }) => {
    this.callTracker.beforeMetricsWritten++;
    return context;
  };

  afterMetricsWritten = async (_context: { metrics: IMetricUpdate[]; targetCollection: string }) => {
  ...
  afterReportGenerated = async (_context: { report: IReportDataPoint[]; query: IQuery }) => {
...
withTestDatabase({ dbName }, async (t, engine) => {
    ...
    await t.step("should execute plugin hooks at various stages", async () => {
        ...
        const eventSource = await engine.createEventSource({ name: "test-source", eventTypes: [{ name: "test-event" }] });
        const report = await engine.createReport({ name: "test-report", active: true });
        ...
        await eventSource.record({ uuid: "test-uuid", eventType: "test-event", payload: { value: 123 } });
        ...
        await engine.getReport({ reportId: report._id.toString(), metric: { type: AggregationType.COUNT }, timeRange: { start: new Date(0), end: new Date() }, granularity: "day" });
    ...
    await t.step("should allow plugins to register custom, type-safe methods on the engine", async () => {
