...
withTestDatabase({ dbName }, async (t, engine) => {
  ...
  const report = await engine.createReport({
    name: "Boolean Group Test Report",
    active: true,
  });

  await engine.addAggregationSource(report._id.toString(), {
    targetCollection: "aggr_dataset_boolean_test",
    granularity: "minute",
    filter: {
      sources: [{ name: "BooleanGroupSource", id: source.getDefinition().id! }],
  ...
  await t.step(
    ...
    async () => {
      ...
      const dataset: IDatasetDataPoint[] = await engine.getDataset(query);
